...
BOOST_AUTO_TEST_CASE(karma_string) {
    std::string string("1");
    BOOST_CHECK_EQUAL(boost::construe_cast<std::string>(1), string);
}

BOOST_AUTO_TEST_CASE(karma_wide_string) {
    std::wstring string(L"1");
    BOOST_CHECK(boost::construe_cast<std::wstring>(1) == string);
}

BOOST_AUTO_TEST_CASE(karma_char_vector) {
    std::vector<char> string(1, '1');
    BOOST_CHECK(boost::construe_cast<std::vector<char> >(1) == string);
}

BOOST_AUTO_TEST_CASE(karma_wide_char_vector) {
    std::vector<wchar_t> string(1, L'1');
    BOOST_CHECK(boost::construe_cast<std::vector<wchar_t> >(1) == string);
}
